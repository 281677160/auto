...
init_var() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -w|--releases_keep_keyword) releases_keep_keyword=($2); shift 2 ;;
            ...
            -k|--workflows_keep_keyword) workflows_keep_keyword=($2); shift 2 ;;
...
get_releases_list() {
    ...
    echo -e "${INFO} 实际总页数: ${actual_total_pages} (Releases实际默认每页最多10个)"
...
get_workflows_list() {
    ...
    echo -e "${INFO} 实际总页数: ${actual_total_pages} (Workflows实际默认每页最多25个)"
    echo -e "${INFO} 程序需要执行的获取页数: ${total_pages_needed} (每页查询${github_per_page}个)"
