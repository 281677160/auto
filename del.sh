...
ensure_file_exists() {
    local file="$1"
    if [[ ! -f "$file" ]]; then
        touch "$file" || error_msg "无法创建文件: $file"
        chmod 600 "$file" || error_msg "无法设置文件权限: $file"
    fi
}
...
init_var() {
    ...
    ensure_file_exists "$filtered_by_keyword"
    ensure_file_exists "$keep_releases_list"
    ensure_file_exists "$all_workflows_list"
    ensure_file_exists "$filtered_by_time"
    ensure_file_exists "$filtered_by_keyword"
    ensure_file_exists "$keep_workflows_list"
